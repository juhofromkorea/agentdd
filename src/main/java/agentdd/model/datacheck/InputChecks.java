...
public final class InputChecks {
    ...
    public static LocalDate date(String value) {
        String s = text(value);
        if (!s.matches("(?:[0-9]{8}|[0-9]{4}-[0-9]{2}-[0-9]{2})")) return null;
        try { return LocalDate.parse(s.replace("-", ""), DateTimeFormatter.BASIC_ISO_DATE); }
        catch (DateTimeParseException e) { return null; }
    }
    private static boolean amount(String value, long max) {
        String s = text(value);
        if (!s.matches("[0-9]{1,18}")) return false;
    ...
    public static Map<String, String> estimate(HttpServletRequest request, boolean draft) {
        ...
        for (String name : new String[] {"nameKana1", "nameKana2"}) if (!v.get(name).isEmpty()) add(e, name, v.get(name).matches("[ァ-ヺー・　 ]+"), "全角カタカナで入力してください。");
        ...
        enums.forEach((name, pattern) -> { if (!v.get(name).isEmpty()) add(e, name, v.get(name).matches(pattern), "選択肢から選び直してください。"); });
        for (String name : new String[] {"inceptionDate", "conclusionDate", "birthday"}) if (!v.get(name).isEmpty()) add(e, name, date(v.get(name)) != null, "有効な日付を入力してください。");
        LocalDate start = date(v.get("inceptionDate")), end = date(v.get("conclusionDate")), birthday = date(v.get("birthday"));
        if (start != null && end != null) add(e, "conclusionDate", end.isAfter(start), "満期日は始期日より後にしてください。");
        if (birthday != null) add(e, "birthday", !birthday.isAfter(LocalDate.now()), "生年月日は本日以前の日付にしてください。");
        if (!v.get("postcode").isEmpty()) add(e, "postcode", v.get("postcode").matches("(?:[0-9]{7}|[0-9]{3}-[0-9]{4})"), "郵便番号は123-4567または1234567の形式で入力してください。");
        for (String name : new String[] {"telephoneNo", "mobilephoneNo", "faxNo"}) {
            String s = v.get(name);
            if (!s.isEmpty()) add(e, name, s.matches("(?:[0-9]{10,11}|[0-9]{2,5}-[0-9]{1,4}-[0-9]{4})") && s.replace("-", "").matches("[0-9]{10,11}"), "電話番号は半角数字10～11桁で入力してください（ハイフン可）。");
        }
        return e;
    }
