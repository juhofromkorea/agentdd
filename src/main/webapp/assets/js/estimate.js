(function () {
    ...
    const database = Array.from(cars.querySelectorAll('option[data-maker]'))
        .map(option => ({ 
    ...
    [...new Set(database.map(car => car.maker))]
        .forEach(name => maker.add(new Option(name, name)));

    function refreshCars() {
        const old = cars.value;
        cars.replaceChildren(new Option('選択してください', ''));

        database.filter(car => !maker.value || car.maker === maker.value)
            .forEach(car => cars.add(new Option(car.label, car.value)));

        if (Array.from(cars.options).some(option => option.value === old)) {
            cars.value = old;
        }
    }
    ...
    const initialCar = database.find(car => (
        car.value === cars.dataset.initial 
        || car.label === cars.dataset.initial) 
        && (!maker.value || car.maker === maker.value));

    if (initialCar) { 
        maker.value = initialCar.maker; 
        refreshCars(); 
        cars.value = initialCar.value; 
    }

    maker.addEventListener('change', refreshCars);
    cars.addEventListener('change', () => {
        const car = database.find(item => 
            item.value === cars.value 
            && (!maker.value || item.maker === maker.value));

        if (car) { 
            maker.value = car.maker; 
            refreshCars(); 
            cars.value = car.value; 
        }
    });
    ...
    function syncParty() {
        ...
        document.querySelectorAll('.estimate-field--personal input, .estimate-field--corporation input')
            .forEach(el => {
                el.disabled = el.closest('.estimate-field--personal') 
                    ? corporate 
                    : !corporate;
        });
    }
    ...
    const fingerprint = () => 
        JSON.stringify(Object.entries(FormErrors.values(form))
    ...
    FormErrors.attach(form, (v, button) => {
        ...
        const required = saving
            ? ['insuredKbn', 'nameKanji1', 'postcode', 'addressKanji1'] 
            : ['insuredKbn', 'nameKanji1', 'nameKana1', 'postcode',
                'addressKanji1', 'addressKana1', 'inceptionDate',
                'conclusionDate', 'inceptionTime', 'conclusionTime',
                'paymentMethod', 'installment', 'maker', 'carName',
                'licenseNo', 'licenseColor', 'ageLimit'];

        if (v.insuredKbn === '1') {
            required.push('nameKanji2');

            if (!saving) {
                required.push('nameKana2', 'gender', 'birthday');
            }
        }

        required.forEach(name => {
            add(name, D.checkRequired(v[name]), 
                '入力・選択してください。');
        });

        if (!D.checkPhoneNoFilledIn(v.telephoneNo, v.mobilephoneNo)) {
            ['telephoneNo', 'mobilephoneNo'].forEach(name => {
                add(name, false, 
        ...
        ['nameKana1', 'nameKana2'].forEach(name => {
            if (D.checkRequired(v[name])) {
                add(name, D.checkCharacterType(v[name], 'katakana'), 
                    '全角カタカナで入力してください。');
            }
        });
        ...
        ['inceptionDate', 'conclusionDate', 'birthday'].forEach(name => {
            if (D.checkRequired(v[name])) {
                add(name, D.checkDate(v[name]), 
                    '有効な日付を入力してください。');
            }
        });

        if (D.checkDate(v.inceptionDate) && D.checkDate(v.conclusionDate)) {
            add('conclusionDate', D.checkInsurancePeriod(v.inceptionDate, v.conclusionDate), 
                '満期日は始期日より後にしてください。');
        }

        if (D.checkRequired(v.birthday)) {
            add('birthday', D.checkBirthday(v.birthday), 
                '生年月日は本日以前の日付にしてください。');
        }
        ...
        if (D.checkRequired(v.telephoneNo)) {
            add('telephoneNo', D.checkPhoneNoFormat(v.telephoneNo, 10),
        ...
        const candidates = { insuredKbn: ['1', '2'], gender: ['1', '2'], 
            licenseColor: ['1', '2', '3'], ageLimit: ['1', '2', '3'], 
            paymentMethod: ['1', '2', '3'], installment: ['1', '6', '12'], 
            inceptionTime: ['09', '10', '11', '12', '13', '14', '15', '16', '17', '18'], 
            conclusionTime: ['09', '10', '11', '12', '13', '14', '15', '16', '17', '18'] };

        Object.entries(candidates).forEach(([name, choices]) => {
            if (D.checkRequired(v[name])) {
                add(name, D.checkClaimMaster(v[name], choices), 
                    '選択肢から選び直してください。');
            }
        });

        if (D.checkRequired(v.maker)) {
            add('maker', database.some(car => car.maker === v.maker), 
                'メーカーを選び直してください。');
        }

        if (D.checkRequired(v.carName)) {
            add('carName', database.some(car => car.value === v.carName && car.maker === v.maker), 
                'メーカーに対応する車名を選択してください。');
        }
        ...
        if (D.checkTransfer(v.paymentMethod, v.installment, policy.paymentCombinations) === false) {
            add('installment', false, 
                '払込方法に対応する払込回数を選択してください。');
        }

        if (D.checkRequired(v.licenseNo) 
                && D.checkCarNumber(v.licenseNo, policy.licenseNumberPattern) === false) {
            add('licenseNo', false, 
                '車両番号の形式を確認してください。');
        }
        ...
        FormErrors.controls(form).forEach(el => {
            if (el.maxLength > -1) {
                add(el.name, D.checkLength(v[el.name], el.maxLength), 
                    el.maxLength + '文字以内で入力してください。');
            }
        ...
        if (printing) {
            if (!D.checkEstimate(calculated)) {
                errors._form = '先に保険料試算を実行してください。';
            } else if (D.checkConditionChanged(baseline, fingerprint())) {
                errors._form = '入力内容が変わりました。保険料を再試算してから印刷してください。';
            }
        }
